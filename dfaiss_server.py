...
from distributed_faiss.server import DEFAULT_PORT
...
def main():
    ...
    parser.add_argument(
        "--num-servers", type=int, required=True, help="how many servers to run"
    )
    parser.add_argument(
        "--save-dir", required=False, default="distributed_faiss_storage"
    )
    parser.add_argument(
        "--num-servers-per-node", type=int, default=10, help="how many servers per node"
    )
    parser.add_argument(
        "--mem-gb", type=int, default=8, help="how much RAM per process"
    )
    parser.add_argument(
        "--cpus-per-node", type=int, default=32, help="how many cpus per node"
    )
    parser.add_argument("--partition", required=True, help="slurm partition")
    parser.add_argument(
        "--timeout-min", type=int, default=60, help="how to run for in min"
    )
    parser.add_argument(
        "--base-port",
        type=int,
